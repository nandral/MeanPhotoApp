(function () {
  ...
  function PhotoViewController($scope, $routeParams, albumService, $location, $rootScope) {
      ...
      if ($scope.photo_next_file) {
        setTimeout(function () {
          $scope.$apply(function () {
            $location.path("/album/" + $scope.album_name + "/photo/" + $scope.photo_next_file);
          });
        }, 200);
      };
    });
    ...
    $rootScope.$on("keypress:37", function (e, ke) {
      if ($scope.photo_prev_file) {
        setTimeout(function () {
          $scope.$apply(function () {
            $location.path("/album/" + $scope.album_name + "/photo/" + $scope.photo_prev_file);
          });
        }, 200);
      }
    });

    albumService.getAlbum($scope.album_name, function (err, album) {
      if (err) {
